...
<nav class="navbar navbar-expand navbar-dark bg-dark">
      ...
      <div class="collapse navbar-collapse" id="navbarsExample02">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item active">
            <a class="nav-link" href="viewcolleagues.php">View Colleagues <span class="sr-only">(current)</span></a>
          </li>
          <li class="nav-item active">
...
<h2 style="text-align:center">Your colleagues</h2>
<br><br><br>
  ...
  <?php
                ...
                $query = "SELECT fname,lname,companyRole,phone FROM employees WHERE companyRole LIKE '%".$_SESSION['companyRole']."%' AND fname !='".$_SESSION['firstName']."' AND lname !='".$_SESSION['lastName']."'" ;
                $result = $conn -> prepare($query);
                $result->execute(
                    array(
                    'fname' => $_SESSION["firstName"],
                ...
                if($result -> rowCount() > 0)
                {
                    while($row = $result->fetch())
                    {
                        echo "</td><td>" .$row["fname"] . "</td><td>". $row["lname"] . "</td><td>". $row["phone"]. "</td><td>". $row["companyRole"] ."</td></tr>";
                    }
                }
             ?>
  </tbody>
